...
def main(base_path):
    ...
    on_time_dataframe = spark.read.parquet(
        "{}/data/on_time_performance.parquet".format(base_path)
    )
  ...
  """
    ...
    sorted_features.repartition(1).write.mode("overwrite").json(
        "{}/data/simple_flight_delay_features.json".format(base_path)
    )
    os.system(
        "cp {}/data/simple_flight_delay_features.json/part* {}/data/simple_flight_delay_features.jsonl".format(
            base_path, base_path
        )
    )
    os.system(
        "bzip2 --best {}/data/simple_flight_delay_features.jsonl".format(base_path)
    )
    os.system(
        "bzcat {}/data/simple_flight_delay_features.jsonl.bz2 >> {}/data/simple_flight_delay_features.jsonl".format(
            base_path, base_path
        )
    )
